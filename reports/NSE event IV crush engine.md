...
The 12 branches above were already grilled on 2026-09-23; the answers are
in section 0A. Do not re-ask them. Copy 0A verbatim into
docs/DESIGN_DECISIONS.md as the first commit, then grill me only on
branches 0A does not cover (and on anything you find ambiguous or in
conflict), and get my confirmation before starting M0.

---------------------------------------------------------------------
0A. DESIGN DECISIONS (BINDING — OVERRIDE ANY CONFLICTING TEXT BELOW)
---------------------------------------------------------------------
D1 Purpose: personal research tool now; paid product later. Live order
   placement out of scope. From day one: tag every data source with its
   licence/redistribution terms; per-user config/results namespaces;
   research core decoupled from UI and data adapters.
D2 Capital & risk: reference account Rs10 lakh (configurable). Max loss
   per event 1% of capital (computed at entry). Max 6 concurrent event
   positions, max 2 per sector. Undefined-risk structures (short
   straddle/strangle, naked ratios) are backtested as benchmarks only and
   never recommended; recommendable = defined-risk only.
D3 Events, release 1: CORPORATE_RESULTS on F&O stocks; RBI_MPC,
   UNION_BUDGET and ELECTION (general-election results) on NIFTY and
   BANKNIFTY. Release 2 (deferred): CPI/GDP India, US FOMC/CPI, index
   rebalances, USFDA, corporate actions, OTHER_SCHEDULED.
D4 Universe & history: point-in-time F&O membership incl. later-removed
   names + NIFTY/BANKNIFTY. Two tiers: every event gets metrics + a
   liquidity grade; only "tradable" events are backtested (initial gate,
   day before entry: event-month ATM strikes >=50 contracts traded and
   OI >=200; next month >= half of that; calibrate later). EOD history
   from 2016; tag settlement regime (cash-settled stock options before
   Jul 2018, phase-in to ~Oct 2019, physical after) and report per
   regime. Intraday: last 5 years only.
D5 Data: free NSE bhavcopy for EOD. Intraday history: try Upstox
   expired-contract APIs first (verify whether Upstox Plus is required via
   a small test pull). Fallback: buy Dhan Data API for one month, run a
   prioritised, resumable bulk pull of event windows only (T-20..T+2, ATM
   +/-10, event and next month, 5-min; macro events first, then liquid
   results, then the rest), respecting rate limits; confirm Dhan terms allow
   retaining data after cancellation. Build the live Upstox option-chain
   quote recorder FIRST (bid/ask/depth/IV; every 5 min normally, 1 min
   on T-1/T/T+1; event-month + next-month chains for stocks reporting in
   the next 3 weeks + NIFTY/BANKNIFTY). Recurring data budget: Rs0 beyond
   broker fees.
D6 Announcement timestamps: NSE corporate-announcement filing timestamp
   of the results outcome = announce_ts; board-meeting intimation gives
   scheduled_date and first_seen_ts; BSE as cross-check. Session mapping:
   <09:15 -> same session; 09:15-15:30 -> intraday, same session;
   >15:30 -> next session; non-trading day -> next session. Manual review
   only for flagged cases (NSE/BSE gap >30 min, missing time, date moved,
   price reaction before filing); budget ~1 hour per results season.
   Unresolved -> UNKNOWN: kept in EOD metrics, excluded from backtests.
D7 Fills: headline = CONSERVATIVE (pay full modelled half-spread each
   leg); mid shown only as optimistic sensitivity; a strategy counts only
   if profitable under conservative fills. Until recorded quotes exist,
   use a labelled prior spread table (% of premium by underlying class,
   moneyness, premium level; index ATM ~0.2-0.5%, liquid stock ATM
   ~1-3%, others 3-8%, min 1-2 ticks; these are assumptions, not
   sourced). 2x spreads in the first 15 min of the reaction session.
   Refit from 1-2 seasons of Upstox snapshots, then re-run and report the
   change. Size cap: 10% of that strike's day volume.
D8 Margin: own 16-scenario SPAN approximation using NSE-published scan
   ranges + exposure margin + expiry-day +2% ELM + no calendar benefit on
   expiry day + upfront premium for longs + delivery-margin escalation for
   stock options. Weekly check against the Upstox margin API on live
   structures; target within +/-15% for defined-risk structures; all
   margin figures labelled approximate. Parse NSE SPAN files in M8 if
   historical files are downloadable.
D9 Strategies, release 1 (8): iron fly (wings ~+/-1.5x implied move);
   iron condor (shorts ~+/-1x implied move, wings one strike step out);
   long calendar (ATM, event month vs next); run-up long straddle and
   run-up long strangle (entry T-7 or T-5, exit pre-announcement);
   hold-through long straddle and hold-through long strangle (T-1 to
   T+1); short straddle as benchmark only. Strangle strikes: 25-delta or
   +/-0.5x implied move. NIFTY/BANKNIFTY macro events use the same set
   where applicable. Deferred: double calendar, BWB, jade lizard, ratios,
   diagonals, directional. Fixed (never tuned): risk limits, liquidity
   gates, fill model, timing grid (entries T-7/T-5/T-3/T-1/pre-announce;
   exits pre-announce/T+1 close/T+2 close). Calibrated on training folds
   only, max 3 values each, all logged: implied/hist-move thresholds
   (priors 1.2 and 0.8), IV_front/IV_next threshold (prior 1.1), wing
   widths.
D10 Statistics: holdout = last 18 months, opened once at the end.
   Walk-forward by results season. Screener may recommend a
   strategy x event class only if ALL: (1) mean after-cost P&L > 0 under
   conservative fills with event-date-clustered t > 3.0; (2) DSR >= 0.95
   counting all logged trials; (3) PBO < 20%; (4) positive in >= 2 of 3
   regimes (pre-Nov 2024 / Nov 2024-Mar 2026 / post-Apr 2026 STT);
   (5) positive in the holdout. Macro events = case studies with CIs,
   never "proven edge". Failures are shown and labelled "not
   significant".
D11 Stack & runtime: research on macOS (Apple Silicon assumed; confirm),
   Python 3.12 via uv, Parquet + DuckDB + polars, Streamlit, no server
   DB; storage budget ~60 GB. Recorder on an always-on Mumbai-region
   Linux VM (try Oracle Cloud Always Free first, else ~Rs500-1,000/month
   VPS) running only the recorder and Upstox token refresh, daily sync to
   the Mac, failure alerts via email or Telegram. Scheduling: cron or
   systemd timers. Code in a new private GitHub repo "nse-event-vol"
   with CI running section 15 tests. Built with Google Antigravity (or
   another agentic IDE): follow section 18.
D12 Verification: the agent researches each section-16 item and drafts
   VERIFICATION_LOG.md entries (URL, circular no., effective date,
   value); I confirm each. BLOCKING for M5+ backtests (not M1-M4): STT
   and exchange charges, lot sizes by date, expiry weekdays, stock-option
   settlement and exercise style, expiry-day margin rules. NON-BLOCKING
   (used with warning banner): RBI/CPI release times, rebalance dates,
   India VIX methodology details, SEBI VWAP consultation. Re-verify
   costs and rules quarterly and after every Union Budget.
...
---------------------------------------------------------------------
Pre-M0 Section 0A copied into DESIGN_DECISIONS.md; follow-up grilling
   (section 0) done and confirmed by me. No code before this.
M0a Upstox live quote recorder (D5) deployed and recording, with
   failure alerts. Built first because unrecorded seasons are lost.
